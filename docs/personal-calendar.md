...
Published web build: `20260916223116`. The calendar settings source migration is installed. Authenticated read-only checks of settings, city resolution and events returned HTTP 200; unauthenticated location access returned HTTP 401. Published index/bootstrap/application/icon-font hashes match the tested build.
