...
const OUT_OF_SCOPE_REPLY =
  'זהו מידע בטוח · AI. השירות יכול לעזור במידע על האפליקציה ובהסבר כיצד להשתמש בה.';
...
const APP_KNOWLEDGE = `
זהות ותחום:
- בתשובה היא אפליקציית תקשורת קהילתית. מידע בטוח · AI משמש גם כמדריך שימוש באפליקציה.

הרשמה, כניסה וחשבון:
- אפשר להירשם ולהיכנס במסכים הייעודיים. אימות עשוי לכלול טלפון ואימייל.
- איפוס סיסמה נעשה דרך "שכחתי סיסמה". לעולם אין למסור לעוזר סיסמה או קוד אימות.
...
אתה "מידע בטוח · AI", מדריך ה-AI האוטומטי של אפליקציית בתשובה.
...
function localGuideAnswer(question, uploadContext = null) {
  ...
  if (/לא (?:עבד|עובד|השתנה|השתנתה|נשמר|נשמרה)|תקלה|שגיאה/.test(q)) {
  ...
  if (/בתשובה|אפליק|הודעה|שיחה|הגדר|עזרה|שלום|היי|מי אתה/.test(q))
    return 'שלום 🌿 זהו מידע בטוח · AI, שירות אוטומטי. אפשר לשאול איך משתמשים בחברים, שיחות, קבוצות, סינון, קבצים, גיבוי, מודעות או הגדרות.';
...
function misunderstandingGuideAnswer(question, history = []) {
  ...
  const description = [
    'שירות ה-AI לא הבין את בקשת המשתמש והחזיר תשובה כללית או חוזרת.',
